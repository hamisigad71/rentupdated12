...
export default function BuildingLedgerModal({
  ...
  const buildingTenants = mockTenants.filter((t) =>
    buildingUnitIds.includes(t.unitId),
  );
  const buildingPayments = mockPayments.filter((p) =>
    buildingUnitIds.includes(p.unitId),
  );
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-[95vw] w-[1100px] sm:max-w-[calc(100vw-2rem)] sm:w-full md:w-[1100px] h-[90vh] p-0 overflow-hidden border-none shadow-2xl rounded-2xl bg-background">
          ...
          <div className="px-6 py-4 flex flex-col md:flex-row items-center justify-between gap-6 border-b border-foreground/5 bg-foreground/[0.02]">
            ...
            <div className="flex items-center gap-4 w-full md:w-auto">
              ...
              <Button
                variant="outline"
                size="icon"
                className="h-12 w-12 rounded-xl border-foreground/10 hover:bg-primary/5 hover:text-primary transition-all"
              >
                    ...
                    <TableBody>
                      {filteredTenants.length > 0 ? (
                        filteredTenants.map((tenant) => (
                          <TableRow
                            key={tenant.id}
                            className="group hover:bg-foreground/[0.01] transition-colors border-b border-foreground/5 last:border-0"
                          >
