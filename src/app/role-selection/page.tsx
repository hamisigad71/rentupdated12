...
import { Building2, Users, ArrowRight, ShieldCheck, Zap, Globe, Star } from "lucide-react";
    ...
    <Link href={href}>
      <div className={cn(
        ...
        <div className={cn(
          ...
          accent ? "bg-primary text-background" : "bg-foreground text-background"
        )}>
          <Icon className="h-10 w-10" strokeWidth={1.5} />
        </div>

        <p className="text-xs font-bold uppercase  text-primary mb-3">{role}</p>
        <h3 className="text-xl md:text-2xl font-bold tracking-tight mb-8 leading-none">{title}</h3>
...
export default function RoleSelection() {
  return (
    <div className="min-h-screen bg-background flex flex-col selection:bg-primary/20 overflow-hidden relative">
      ...
      <div className="absolute top-0 right-0 w-[50vw] h-[50vh] bg-primary/5 rounded-full blur-[160px] -mr-[25vw] -mt-[25vh] pointer-events-none" />
      <div className="absolute bottom-0 left-0 w-[50vw] h-[50vh] bg-primary/5 rounded-full blur-[160px] -ml-[25vw] -mb-[25vh] pointer-events-none" />

      <header className="container relative z-10 px-6 py-6 flex justify-between items-center">
        <Logo size="lg" />
        <Link href="/auth/login">
           <Button variant="ghost" className="text-xs font-bold uppercase  hover:bg-foreground/5 transition-all">
             Abort <ArrowRight className="ml-2 h-3 w-3" />
           </Button>
        </Link>
      </header>

      <main className="flex-1 container relative z-10 px-6 flex flex-col items-center justify-center py-10">
        ...
        <div className="mt-24 pt-12 border-t border-foreground/5 w-full max-w-4xl grid grid-cols-2 md:grid-cols-4 gap-6 text-center opacity-40 hover:opacity-100 transition-opacity duration-1000">
           {[
             ...
             <div key={i} className="space-y-2">
                <p className="text-[8px] font-bold uppercase  text-muted-foreground">{stat.l}</p>
